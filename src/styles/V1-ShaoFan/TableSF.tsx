...
import { ArrowLeftRight, PencilSquare, SortAlphaDown, SortAlphaUpAlt, Trash3 } from "react-bootstrap-icons";
          ...
          {data &&
            data.map((row, i) => (
              <tr key={i} className={i%2 === 0 ? "" : "table-bg"}>
                ...
                {editIcon && <td width={1}><PencilSquare className="icon"/></td>}
                {deleteIcon && <td><Trash3 className="icon"/></td>}
              </tr>
            ))}
